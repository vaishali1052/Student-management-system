...
def student_feedback_save(request):
    if request.method != "POST":
        ...
        try:
            ...
            print("Student feedback save error:", e)
...
def student_send_staff_feedback(request):
    staffs = Staffs.objects.all()
    student = Students.objects.get(admin=request.user.id)
    sent_feedbacks = StudentToStaffFeedback.objects.filter(student_id=student)
    return render(request, "student_template/student_staff_feedback.html", {
        "staffs": staffs,
        "sent_feedbacks": sent_feedbacks,
    })


def student_send_staff_feedback_save(request):
    if request.method != "POST":
        return HttpResponseRedirect(reverse("student_send_staff_feedback"))
    ...
    try:
        ...
        StudentToStaffFeedback.objects.create(
            student_id=student,
            staff_id=staff,
            feedback=feedback_msg.strip(),
            feedback_reply=""
        )
        messages.success(request, f"Feedback sent to {staff.admin.username} successfully!")
        return HttpResponseRedirect(reverse("student_send_staff_feedback"))
    except Exception as e:
        print("Error:", e)
        messages.error(request, "Failed to send feedback")
